...
myAudio.volume = elem_volume.value;  // 初期値設定
// ボリューム変更時
elem_volume.addEventListener("change", function(){
	myAudio.volume = elem_volume.value;
}, false);
...
class HiveWhale extends Enemy {
    constructor(game){
        ...
        this.score = this.lives;
        this.type = 'hive';
        this.speedX = Math.random() * -0.6 - 0.6;
    }
}
class Drone extends Enemy {
    constructor(game, x, y){
        ...
        this.score = this.lives;
        this.type = 'drone';
        this.speedX = Math.random() * -2.2 - 1.5;
    }
}
    class Layer {
    ...
    class Explosion {
        ...
        draw(context){
            context.drawImage(this.image, this.frameX * this.spriteWidth,  0, this.spriteHeight, this.spriteWidth, 
                                this.x, this.y, this.width, this.height);
        }
    }
        ...
        draw(context){
            ...
            if (this.game.gameOver){
                ...
                if (this.game.score > this.game.winningScore){
                    messages1 = 'YOU WIN!';
                    messages2 = 'Congratulations!';
                } else {
                    messages1 = 'GAME OVER!';
                    messages2 = 'Try again!';
                }
                context.font = '80px ' + this.fontFamily;
                context.fillText(messages1 , this.game.width * 0.5, this.game.height * 0.5 - 20);
                context.font = '35px ' + this.fontFamily;
                context.fillText(messages2 , this.game.width * 0.5, this.game.height * 0.5 + 20);
            }
                //ammo
    ...
    class Game {
        ...
        addEnemy(){
            const randomze = Math.random();
            if (randomze < 0.3) this.enemies.push(new Angler1(this));
            else if (randomze < 0.6 ) this.enemies.push(new Angler2(this));
            else if (randomze < 0.7 ) this.enemies.push(new HiveWhale(this));
